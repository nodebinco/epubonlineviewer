<script lang="ts">
	...
	$effect(() => {
		...
		async function loadBook() {
			...
			loadedMetadata = data.metadata
				? {
						lastPosition: data.metadata.lastPosition,
						lastCfi: data.metadata.lastCfi,
						lastHref: data.metadata.lastHref
					}
				: null;
			if (data.metadata) {
				if (
					typeof data.metadata.fontSize === 'number' &&
					data.metadata.fontSize >= 90 &&
					data.metadata.fontSize <= 150
				) {
		...
		initRafId = requestAnimationFrame(() => {
			...
			rend.on(
				...
				(loc: {
					...
					const cfiStr =
						rawCfi != null
							? typeof rawCfi === 'string'
								? rawCfi
								: ((rawCfi as { toString?: () => string }).toString?.() ?? '')
							: undefined;
			...
			if (hasValidCfi) {
				...
				whenReady
					.then(() =>
						rend.display(lastCfi!).catch(() => {
							throw new Error('CFI restore failed');
						})
					)
					.then(() => {
						setTimeout(() => {
					...
					.catch(() => {
						if (hasValidHref) {
							...
							(bk.locations?.generate?.() ?? Promise.resolve())
								.catch(() => {})
								.then(() => {
	...
	function savePositionOnLeave(): Promise<void> {
		...
		if (typeof pct === 'number' && pct >= 0 && pct <= 1) {
			return updateBookMetadata(id, {
				lastOpened: Date.now(),
				lastPosition: pct,
				lastCfi: lastCfi,
				lastHref: lastHrefVal
			});
		}
...
<div class="bg-base-100 flex h-full min-h-0 flex-col overflow-hidden">
	{#if loading}
	...
	{:else if bookData}
		...
		<div class="reader-page bg-base-100 flex h-full min-h-0 flex-col overflow-hidden">
			<!-- Toolbar: sticky so it stays fixed when scrolling -->
			<nav
				class="reader-toolbar bg-base-200 border-base-300 sticky top-0 z-10 grid h-14 shrink-0 grid-cols-[1fr_auto_1fr] items-center gap-2 border-b px-2 md:px-3"
				aria-label="Reader toolbar"
			>
				<div class="menu-1 flex min-w-0 items-center justify-start gap-1">
					<a
						href={homeHref}
						class="btn btn-sm btn-ghost btn-square"
						title={m.back_to_library()}
						aria-label={m.back_to_library()}
					>
						<!-- Lucide ArrowLeft -->
						<svg
					...
					<button
						...
						aria-label={m.reader_toc_toggle()}
				...
				<div
					class="menu-center flex max-w-[min(100%,22rem)] shrink-0 items-center justify-center gap-2"
					aria-label={title || 'Book title'}
				>
					<span class="text-base-content/80 truncate text-sm whitespace-nowrap">{title || '—'}</span
					>
				</div>
				<div
					class="menu-2 flex min-w-0 items-center justify-end gap-1"
					role="group"
				...
				>
					...
					<details class="dropdown dropdown-end hidden sm:block">
						<summary
							...
							onclick={(e) =>
								closeOtherToolbarDropdowns((e.currentTarget as HTMLElement).closest('details'))}
							><span class="text-sm">Spread</span></summary
						>
						<ul
							role="listbox"
							class="dropdown-content menu bg-base-100 rounded-box border-base-300 z-50 mt-1 w-40 border p-2 shadow"
						>
							<li>
								<button
									type="button"
									class="text-sm {spreadMode === 'single' ? 'active' : ''}"
									onclick={() => setSpreadMode('single')}>Single page</button
								>
							</li>
							<li>
								<button
									type="button"
									class="text-sm {spreadMode === 'double' ? 'active' : ''}"
									onclick={() => setSpreadMode('double')}>Two pages</button
								>
							</li>
						</ul>
					</details>
					<details class="dropdown dropdown-end hidden sm:block" id="reader-font-family-dropdown">
						<summary
							class="btn btn-sm btn-ghost gap-1"
							title={m.reader_font_family()}
							onclick={(e) =>
								closeOtherToolbarDropdowns((e.currentTarget as HTMLElement).closest('details'))}
							><span class="text-sm">{m.reader_font_family()}</span></summary
						>
						<ul
							role="listbox"
							class="dropdown-content menu menu-vertical bg-base-100 rounded-box border-base-300 z-50 mt-1 max-h-64 w-52 flex-nowrap overflow-y-auto border p-2 shadow"
						>
							{#each FONT_FAMILIES as fam}
								<li class="w-full">
									<button
										type="button"
										class="w-full rounded-md px-3 py-2 text-left text-sm {readerFontFamily ===
										fam.value
											? 'active'
											: ''}"
										onclick={() => (readerFontFamily = fam.value)}>{fam.label()}</button
									>
								</li>
							{/each}
						</ul>
					...
					<details class="dropdown dropdown-end hidden sm:block" id="reader-font-size-dropdown">
						<summary
							class="btn btn-sm btn-ghost gap-1"
							title={m.reader_font_size()}
							onclick={(e) =>
								closeOtherToolbarDropdowns((e.currentTarget as HTMLElement).closest('details'))}
							><span class="text-sm">{readerFontSize}%</span></summary
						>
						<ul
							role="listbox"
							class="dropdown-content menu bg-base-100 rounded-box border-base-300 z-50 mt-1 w-32 border p-2 shadow"
						>
							{#each FONT_SIZES as pct}
								<li>
									<button
										type="button"
										class="text-sm {readerFontSize === pct ? 'active' : ''}"
										onclick={() => (readerFontSize = pct)}>{pct}%</button
									>
								</li>
							{/each}
						</ul>
					...
					<button
						...
						title={fullScreen ? m.reader_exit_full_screen() : m.reader_full_screen()}
						aria-label={fullScreen ? m.reader_exit_full_screen() : m.reader_full_screen()}
					>
						{#if fullScreen}
							<!-- Lucide Minimize2 -->
							<svg
								...
								stroke-linejoin="round"
								><path
									d="M8 3v3a2 2 0 0 1-2 2H3m18 0h-3a2 2 0 0 1-2-2V3m0 18v3a2 2 0 0 0 2 2h3M3 16h3a2 2 0 0 0 2-2v-3"
								/></svg
							>
						{:else}
							<!-- Lucide Maximize2 -->
			...
			<div class="flex min-h-0 flex-1 overflow-hidden">
				<aside
					class="bg-base-200 border-base-300 flex min-h-0 w-64 shrink-0 flex-col overflow-hidden border-r transition-[margin,width] duration-200 {tocOpen
						? 'flex'
						: 'hidden'}"
					aria-label={m.reader_toc()}
					aria-hidden={!tocOpen}
				>
					<div class="border-base-300 shrink-0 border-b p-3 text-sm font-semibold">
						{m.reader_toc()}
					</div>
					<nav
						...
						aria-label={m.reader_toc()}
					>
						<ul class="menu menu-sm w-full gap-0.5 p-0" bind:this={tocListEl}>
							{#each flatToc as chapter (chapter.id ?? '' + chapter.href)}
								<li class="w-full">
									<button
										type="button"
										data-href={chapter.href}
										class="w-full rounded-md px-3 py-2 text-left text-sm {currentHref ===
										chapter.href
											? 'btn-active bg-primary/15 text-primary font-medium'
											: 'hover:bg-base-300'}"
				...
				<div
					...
					class="reader-content flex min-w-0 flex-1 flex-col overflow-hidden outline-none reader-theme-{readerTheme}"
					tabindex="0"
				...
				>
					<div class="flex min-h-0 flex-1 items-stretch overflow-hidden">
						<button
							type="button"
							class="reader-arrow prev flex min-w-[48px] flex-shrink-0 items-center justify-start px-2 md:px-4 {!canPrev
						...
						>
							<span
								class="text-base-content/40 hover:text-base-content/70 text-4xl font-bold select-none md:text-5xl lg:text-6xl"
								>&lt;</span
							>
						</button>
						<div
							class="viewer-container relative flex min-w-0 flex-1 items-center justify-center overflow-hidden reader-theme-bg-{readerTheme}"
						>
							<div bind:this={viewerEl} class="h-full min-h-[400px] w-full"></div>
							{#if restoringPosition}
								<div
									class="absolute inset-0 z-10 flex items-center justify-center reader-restoring-overlay-{readerTheme}"
									aria-live="polite"
									aria-busy="true"
								>
						...
						<button
							type="button"
							class="reader-arrow next flex min-w-[48px] flex-shrink-0 items-center justify-end px-2 md:px-4 {!canNext
								? 'pointer-events-none opacity-30'
								: 'hover:bg-base-200'}"
							onclick={goNext}
							title={m.reader_next()}
