...
<div class="bg-base-100 flex min-h-screen flex-col">
	{#if !page.url.pathname.startsWith('/reader')}
		...
		<header class="navbar bg-base-200 border-base-300 sticky top-0 z-50 shrink-0 border-b">
			<div
				class="mx-auto flex w-full max-w-[1920px] items-center justify-between gap-2 px-2 md:px-3"
			>
				<div class="navbar-start min-w-0 gap-2">
					<details class="dropdown lg:hidden" bind:open={navOpen}>
						...
						<ul class="menu dropdown-content bg-base-100 rounded-box z-50 mt-2 w-56 p-2 shadow">
							<li>
								<a href={localizeHref('/getting-started', { locale: displayLocale ?? baseLocale })}
									>{m.nav_getting_started({ locale: displayLocale })}</a
								>
							</li>
							<li>
								<a href={localizeHref('/what-is-epub', { locale: displayLocale ?? baseLocale })}
									>{m.nav_what_is_epub({ locale: displayLocale })}</a
								>
							</li>
							<li>
								<a href={localizeHref('/tools', { locale: displayLocale ?? baseLocale })}
									>{m.nav_tools({ locale: displayLocale })}</a
								>
							</li>
							<li>
								<a href={localizeHref('/', { locale: displayLocale ?? baseLocale })}
									>{m.nav_open_epub_reader({ locale: displayLocale })}</a
								>
							</li>
						</ul>
					</details>
					<a
						href={localizeHref('/', { locale: displayLocale ?? baseLocale })}
						class="brand-link hover:bg-base-300/50 flex items-center gap-2.5 rounded-lg px-2 py-1.5 transition-colors"
					>
				...
				<div class="navbar-end gap-1">
					...
					<div class="dropdown dropdown-end">
						...
						<div
							tabindex="0"
							role="menu"
							class="dropdown-content bg-base-100 rounded-box z-50 mt-2 max-h-[70vh] w-auto min-w-[280px] overflow-y-auto p-2 shadow"
						>
							<div class="grid grid-cols-2 gap-1 sm:grid-cols-3">
								{#each locales as locale}
									<a
										...
										role="menuitem"
										class="hover:bg-base-200 focus:bg-base-200 rounded-lg px-3 py-2 text-sm focus:outline-none {locale ===
										displayLocale
		...
		<div
			class="mx-auto flex min-h-0 w-full max-w-[1920px] flex-1 flex-col overflow-hidden px-2 md:px-3"
		>
			<main class="flex min-h-0 min-w-0 flex-1 flex-col overflow-auto pt-14 md:pt-16">
				{#key localeKey}
					{@render children()}
				{/key}
			</main>
		...
		<footer
			class="footer footer-center bg-base-200 border-base-300 text-base-content/80 border-t p-6 text-sm"
		>
			<div class="flex flex-wrap justify-center gap-4 md:gap-6">
				<a
					href={localizeHref('/tools', { locale: displayLocale ?? baseLocale })}
					class="link link-hover">{m.nav_tools({ locale: displayLocale })}</a
				>
				<a
					href={localizeHref('/about-us', { locale: displayLocale ?? baseLocale })}
					class="link link-hover">{m.footer_about({ locale: displayLocale })}</a
				>
				<a
					href={localizeHref('/privacy-policy', { locale: displayLocale ?? baseLocale })}
					class="link link-hover">{m.footer_privacy({ locale: displayLocale })}</a
				>
				<a
					href={localizeHref('/terms-of-use', { locale: displayLocale ?? baseLocale })}
					class="link link-hover">{m.footer_terms({ locale: displayLocale })}</a
				>
				<a
					href={localizeHref('/contact-us', { locale: displayLocale ?? baseLocale })}
					class="link link-hover">{m.footer_contact({ locale: displayLocale })}</a
				>
			</div>
		</footer>
	{:else}
		<!-- Reader: exactly viewport height, no page scroll; TOC and content scroll independently inside -->
		<div class="mx-auto flex h-screen max-h-dvh w-full max-w-[1920px] flex-col overflow-hidden">
